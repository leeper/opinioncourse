...
\item Two equivalent ways to obtain a t-statistic:
	...
	\end{itemize}
\item (See regression slides to understand the math involved)
\end{itemize}
}
...
\frame{
\begin{center}
\includegraphics[height=\textwidth]{images/power}
\end{center}
}
...
\item We can ignore non-compliance and analyze data ``as-assigned'', but this ``intention to treat'' effect will underestimate our effects because some people were not treated as assigned\\
	$ITT = \overline{Y}_1 - \overline{Y}_0$
\item We can use ``instrumental variables'' analysis and estimate the ``local average treatment effect'' (LATE) for those that complied with treatment:\\
